 ...
 */
public class UserWordExerciseDynamicSqlProvider {

    public String queryUserExerciseAssociateAnnotation(final UserWordExercise userExercise){
        return new SQL(){
            {
                ...
                FROM("user_word_exercise ue ");
                INNER_JOIN("user_account ua on ue.user_modifier=ua.id ");
                INNER_JOIN("annotation_word_exercise asent on ue.anId=asent.id ");
                WHERE("1=1");
                if(StringUtils.isNotBlank(userExercise.getState()))
                    WHERE("ue.state=#{state}");
                if(userExercise.getUserModifier()>0)
                    WHERE("ue.user_modifier=#{userModifier}");
            }
        }.toString();
    }

    public String queryUserWordExerciseAnnotation(final UserWordExercise userExercise){
        return new SQL(){
            {
    ...
    //根据唯一索引批量更新
    public String batchUpdateUserWordExerciseUser(Map map){
        List<UserWordExercise> userExercisesList=(List<UserWordExercise>)map.get("userWordExerciseList");
        StringBuilder sb=new StringBuilder();
        sb.append("update user_word_exercise set practice_annotation = case");
        for(UserWordExercise userExercises:userExercisesList){
    ...
    public String batchAddUserWordExercise(Map map){
        List<UserWordExercise> userExercisesList=(List<UserWordExercise>)map.get("list");
        StringBuilder sb=new StringBuilder();
        sb.append("insert into user_word_exercise ");
    ...
    public String updateUserWordExerciseSelective(final UserWordExercise userExercises){
        return new SQL(){
            {
                UPDATE("user_word_exercise");
