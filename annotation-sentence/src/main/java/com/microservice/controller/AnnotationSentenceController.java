...
public class AnnotationSentenceController extends BaseController {
  ...
  public ResultVO<PageVO<AnnotationSentenceBratVO>> queryAnnotationSentence(
      @RequestBody JSONObject jsonParam, @ModelAttribute("userAccount") UserAccount userAccount) {
    int pageIndex = jsonParam.getIntValue("pageIndex");
    int pageSize = jsonParam.getIntValue("pageSize");
    List<String> stateList = JSON
        .parseArray(JSON.toJSONString(jsonParam.get("states")), String.class);
//        String userModifier=jsonParam.getString("userModifier");

    Page<AnnotationSentence> pageInfo = annotationSentenceService
        .queryAnnotationByCondition(pageIndex, pageSize, stateList,
    ...
    if (pageInfo.getResult().size() == 0) {
      ...
      pageInfo = annotationSentenceService
          .queryAnnotationUnDistribution(pageIndex, pageSize, stateList);
      if (pageInfo.getResult().size() > 0) {
        //将当前未分配的标注授权给当前的用户
        AnnotationSentence autoDistributionAnnotationSentence = pageInfo.get(0);
        autoDistributionAnnotationSentence
            .setState(AnnotationSentenceStateEnum.DISTRIBUTIONED.getMessage());
    ...
    List<AnnotationSentenceBratVO> annotationSentenceBratVOList = AnnotationConvert
        .convert2AnnotationBratVOList(pageInfo.getResult());
  ...
  public ResultVO<PageVO<AnnotationSentenceBratVO>> queryAnnotationSentence2Distribution(
    ...
    String userModifier = jsonParam.getString("userModifier");
    Page<AnnotationSentence> pageInfo = annotationSentenceService
        .queryAnnotationByCondition(pageIndex, pageSize, stateList, userModifier);
    List<AnnotationSentenceBratVO> annotationSentenceBratVOList = AnnotationConvert
        .convert2AnnotationBratVOList(pageInfo.getResult());
    PageVO<AnnotationSentenceBratVO> pageVO = new PageVO(pageInfo, false);
    pageVO.setDataList(annotationSentenceBratVOList);
  ...
  public ResultVO<AnnotationSentenceBratVO> getAnnotationById(@RequestBody JSONObject jsonParam) {
    ...
    AnnotationSentenceBratVO annotationSentenceBratVO = AnnotationConvert
        .convert2AnnotationBratVO(annotationSentence);
  ...
  public ResultVO<AnnotationSentenceBratVO> addAnnotationSentence(@RequestBody JSONObject jsonParam,
    ...
    if (userAccount.getRole().equals("标注人员")) {
      newAnnotationSentence = AnnotationConvert
          .addUnitAnnotation(annotationSentence.getAnnotationText(),
              type, startPosition, endPosition, term);
    } else {
      newAnnotationSentence = AnnotationConvert
          .addUnitAnnotation(annotationSentence.getFinalAnnotationText(),
              type, startPosition, endPosition, term);
    }
    ...
    AnnotationSentenceBratVO finalAnnotationSentenceBratVO = updateAnnotationSentenceAnnotationText(
        anId, newAnnotationSentence, role);
...
//    }
  ...
  public ResultVO<AnnotationSentenceBratVO> deleteAnnotationSentence(
      @RequestBody JSONObject jsonParam, @ModelAttribute("userAccount") UserAccount userAccount) {
    int anId = jsonParam.getIntValue("id");
    String tag = jsonParam.getString("tag");
    ...
    if (userAccount.getRole().equals("标注人员")) {
      newAnnotationSentence = AnnotationConvert
          .deleteUnitAnnotation(annotationSentence.getAnnotationText(), tag);
    } else {
      newAnnotationSentence = AnnotationConvert
          .deleteUnitAnnotation(annotationSentence.getFinalAnnotationText(), tag);
    }
    int role = 0;//默认为标注人员
    if (userAccount.getRole().equals("管理员") || userAccount.getRole().equals("审核人员")) {
      role = 1;
  ...
  public ResultVO<AnnotationSentenceBratVO> updateFinalAnnotation(@RequestBody JSONObject params,
      @ModelAttribute("userAccount") UserAccount userAccount) {
...
//        String oldType=params.getString("oldType");
    ...
    if (userAccount.getRole().equals("标注人员")) {
      finalAnnotation = AnnotationConvert
          .changeUnitAnnotation(annotation.getAnnotationText(), tag, newType);
    } else {
      finalAnnotation = AnnotationConvert
          .changeUnitAnnotation(annotation.getFinalAnnotationText(), tag, newType);
    }
    int role = 0;//默认为标注人员
    if (userAccount.getRole().equals("管理员") || userAccount.getRole().equals("审核人员")) {
      role = 1;
    ...
    //更新单条标注信息到数据库
    AnnotationSentenceBratVO annotationBratVO = updateAnnotationSentenceAnnotationText(anId,
        finalAnnotation, role);
   ...
   */
  private AnnotationSentenceBratVO updateAnnotationSentenceAnnotationText(int anId,
      String newAnnotationText, int role) {
   ...
   * 重置审核标注回用户的那个状态
   */
  @RequestMapping(value = "/resetAnnotationSentence.do")
  public ResultVO<AnnotationSentenceBratVO> resetAnnotationSentence(
      @RequestBody JSONObject jsonParam) {
    int anId = jsonParam.getIntValue("id");
    AnnotationSentence annotationSentence = annotationSentenceService.getAnnotationSentence(anId);
    annotationSentence.setFinalAnnotationText(annotationSentence.getAnnotationText());
    annotationSentenceService.updateAnnotationSentence(annotationSentence);
    AnnotationSentence finalAnnotationSentence = annotationSentenceService
        .getAnnotationSentence(anId);
    return ResultVO.success(AnnotationConvert.convert2AnnotationBratVO(finalAnnotationSentence));
  }
  ...
  public ResultVO commitAnnotationSentence(@RequestBody JSONObject jsonParam) {
    int anId = jsonParam.getIntValue("id");
    AnnotationSentence oldAnnotationSentence = annotationSentenceService
        .getAnnotationSentence(anId);
