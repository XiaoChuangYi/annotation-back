...
public class DesignateUserExerciseBiz extends BaseBiz<DesignateAnnotationRequest, Object> {
  ...
  public DesignateUserExerciseBiz(
  ...
  protected void validateRequest(DesignateAnnotationRequest designateAnnotationRequest)
    ...
    if (designateAnnotationRequest.getIdList() == null
        && designateAnnotationRequest.getIdList().size() == 0) {
  ...
  protected Object doBiz(DesignateAnnotationRequest designateAnnotationRequest) {
    List<AnnotationCombine> annotationCombineList =
        annotationCombineRepository.findAllByIdInAndIsTaskEquals(
            designateAnnotationRequest.getIdList(), 1);
    // 再加一层过滤，如果前端把已经指派过的标准习题集再次指派，则过滤该id
    List<UserExercise> userExerciseList =
        userExerciseRepository.findAllByAssigneeEquals(designateAnnotationRequest.getUserId());
    Iterator<AnnotationCombine> iterator = annotationCombineList.iterator();
    while (iterator.hasNext()) {
      AnnotationCombine annotationCombine = iterator.next();
      if (userExerciseList
              .stream()
              .filter(userExercise -> userExercise.getAnnotationId() == annotationCombine.getId())
              .count()
          > 0) {
        iterator.remove();
      }
