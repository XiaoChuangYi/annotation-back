...
public class AnnotationRelevantConvert {
   ...
   */
  public static List<AnnotationBratVO> convert2AnnotationBratVOList(
      List<AnnotationWordPos> annotationWordPosList) {
    ...
    if (annotationWordPosList.size() > 0) {
      for (AnnotationWordPos annotation : annotationWordPosList) {
        ...
        JSONObject bratJson = toConvertAnnotation2BratFormat(annotation.getTerm(),
   ...
   * 新增relation
   */
  public static String addRelationAnnotation(String oldAnnotation, String sourceTag,
  ...
  public static String updateRelationAnnotation(String oldAnnotation, String rTag, String newType) {
    ...
    if (annoDocument.getRelationEntities().size() > 1 && checkRelationRepetition(
        annoDocument.getRelationEntities())) {
      return oldAnnotation;
    }
    return toBratAnnotations(annoDocument);
    ...
    long count = IntStream.range(0, relationEntityList.size())
        .filter(i ->
            relationEntityList.stream()
                .anyMatch(
                    x -> !x.getTag().equals(relationEntityList.get(i).getTag()) && x.getSourceTag()
