...
public class AnnotationWordPosExerciseService {
  ...
  @Autowired private AnnotationWordPosExerciseMapper annotationWordPosExerciseMapper;

  @Autowired private UserWordExerciseMapper userWordExerciseMapper;
  ...
  public void updateAnnotationWordPosExerciseSelective(
      AnnotationWordPosExercise annotationWordPosExercise) {
  ...
  public void initUserExercises(int userModifier) {
    ...
    if (sum == 0) {
      ...
      List<AnnotationWordPosExercise> annotationSentenceExerciseList =
          annotationWordPosExerciseMapper.listAnnotationWordExerciseAll();
      List<UserWordExercise> userExercisesList =
          IntStream.range(0, annotationSentenceExerciseList.size())
              .mapToObj(
                  i ->
                      new UserWordExercise(
                          annotationSentenceExerciseList.get(i).getOriginText(),
  ...
  /** 查询标准练习数据 */
  public Map<String, Object> listAnnotationWordExercise(
      int pageIndex, int pageSize, int userModifier, String state) {
    List<AnnotationWordPosExercise> annotationSentenceExerciseList =
        annotationWordPosExerciseMapper.listAnnotationWordExerciseAll();
    ...
    List<UserWordExercise> userExercisesList =
        userWordExerciseMapper.queryUserWordExerciseAnnotation(paramUserExercises);

    List<AnnotationWordPosExercise> finalAnnotationList;
    for (UserWordExercise current : userExercisesList) {
      annotationSentenceExerciseList
          .stream()
          .filter(x -> x.getId() == current.getAnId())
          .forEach(
              k -> {
                k.setMemo("匹配");
                k.setState(current.getState());
              });
      //            finalAnnotationList.addAll(annotationSentenceExerciseList.stream().filter(x ->
      // x.getId() == current.getAnId()).collect(Collectors.toList()));
    }
    if (StringUtils.isNotBlank(state)) {
      finalAnnotationList =
          annotationSentenceExerciseList
              .stream()
              .filter(x -> state.equals(x.getState()))
              .collect(Collectors.toList());
    } else {
  ...
  public void designateAnnotationWordExercise(
      List<Integer> anIdArr, int userModifier, String state) {
    String idTemp = anIdArr.stream().map(x -> x.toString()).collect(Collectors.joining(","));

    List<UserWordExercise> userExercisesList =
        userWordExerciseMapper.listUserWordExerciseByUserModifier(userModifier);
    List<AnnotationWordPosExercise> annotationSentenceExerciseList =
        annotationWordPosExerciseMapper.listAnnotationWordExerciseByIdArr(idTemp);
    ...
    Date currentDate = new Date();
    List<UserWordExercise> finalUserExercisesList =
        IntStream.range(0, annotationSentenceExerciseList.size())
            .mapToObj(
                i ->
                    new UserWordExercise(
  ...
  private List<UserWordExercise> convert2UserExercises(
      List<AnnotationWordPosExercise> annotationSentenceExerciseList,
      String state,
      int userModifier) {
  ...
  /** 查询标准习题集 */
  public Page<AnnotationWordPosExercise> listAnnotationSentExerciseByPaging(
      int pageIndex, int pageSize, String originText) {
    ...
    annotationWordPosExerciseMapper.listAnnotationWordExerciseByCondition(
        annotationWordPosExercise);
    return pageInfo;
  }
}
