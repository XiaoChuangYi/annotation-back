 ...
 */
public class AnnotationNegationConvert {


    public static Logger logger = Logger.getLogger(AnnotationNegationConvert.class);
     ...
     */
    public static List<AnnotationNegationBratVO> convert2AnnotationBratVOList(List<AnnotationNegation> annotationList){
        List<AnnotationNegationBratVO> annotationBratVOList=new LinkedList<>();
        for(AnnotationNegation annotation:annotationList){
            JSONObject bratJson=convertToBratFormat(annotation.getOriginText(),annotation.getAnnotationText());
            AnnotationNegationBratVO annotationNegationBratVO=new AnnotationNegationBratVO();
            BeanUtils.copyProperties(annotation,annotationNegationBratVO);
            annotationNegationBratVO.setBratData(bratJson);
            annotationBratVOList.add(annotationNegationBratVO);
        }
     ...
     * 将标注数据部分字段转换成前端可以渲染的数据格式
     */
    public static AnnotationNegationBratVO convert2AnnotationBratVO(AnnotationNegation annotation){
        JSONObject bratJson=convertToBratFormat(annotation.getOriginText(),annotation.getAnnotationText());
        AnnotationNegationBratVO annotationNegationBratVO=new AnnotationNegationBratVO();
        BeanUtils.copyProperties(annotation,annotationNegationBratVO);
        annotationNegationBratVO.setBratData(bratJson);
        return annotationNegationBratVO;
    }
