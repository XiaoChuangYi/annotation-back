...
public class ResetUserExerciseBiz extends BaseBiz<UserResetRequest,Object> {
  ...
  protected Object doBiz(UserResetRequest resetRequest) {
    List<UserExercise> userExerciseList=userExerciseRepository.findAllByAssigneeEquals(resetRequest.getUserId());
    List<Integer> annotationIdList=userExerciseList.stream().map(userExercise -> userExercise.getAnnotationId()).collect(
        Collectors.toList());
    List<AnnotationCombine> annotationCombineList=annotationCombineRepository.findAllByIdInAndIsTaskEquals(annotationIdList,1);
    Map<Integer,String> map=new HashMap<>();
    //暂定练习模块的答案习题集预标注是字段final_annotation
    IntStream.range(0,annotationCombineList.size()).forEach(i->{
      map.put(annotationCombineList.get(i).getId(),annotationCombineList.get(i).getFinalAnnotation());
    });
    userExerciseList.stream().forEach(userExercise -> userExercise.setUserAnnotation(map.get(userExercise.getAnnotationId())));
