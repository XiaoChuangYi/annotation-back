 ...
 */
@Component
public class GetAnnotationSummaryByAssigneeBiz extends BaseBiz<SetUserStateRequest,List<AnnotationSummaryVO>> {
  ...
  @Override
  protected List<AnnotationSummaryVO> doBiz(SetUserStateRequest setUserStateRequest) {
    ...
    if(globalRole==3){//标注人员
      annotationSummaryList=annotationCombineRepository.findByAssigneeAndStateGroup(setUserStateRequest.getUserId());
      finalAnnotationSummaryVOList = annotationSummaryList.stream()
          .map(x -> new AnnotationSummaryVO(x.getState(), x.getNum())).collect(
              Collectors.toList());
    }
    if(globalRole==4){//练习人员
      annotationSummaryList=userExerciseRepository.findByAssigneeAndStateGroup(setUserStateRequest.getUserId());
      finalAnnotationSummaryVOList = annotationSummaryList.stream()
          .map(x -> new AnnotationSummaryVO(x.getState(), x.getNum())).collect(
              Collectors.toList());
    }
